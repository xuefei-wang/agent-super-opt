...
def visualize_diverse_samples(
    npz_path: str, output_dir: str = "visualizations", indices_path: str = None
) -> None:
    ...
    images = (
        dataset.load_all()
    )  # Will return ImageData objects with standardized formats
    ...
    for i, img in enumerate(images):
        ...
        fig.suptitle(
            f"Sample {i}"
            + (
                f" (Original Index: {selected_indices[i]})"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--npz_path", required=True, help="Path to diverse_samples.npz file"
    )
    parser.add_argument(
        "--output_dir",
    ...
    parser.add_argument(
        "--indices_path", default=None, help="Optional path to selected_indices.npy"
    )

    args = parser.parse_args()

    visualize_diverse_samples(args.npz_path, args.output_dir, args.indices_path)
