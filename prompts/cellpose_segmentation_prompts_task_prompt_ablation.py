...
class CellposeSegmentationPromptsWithSkeleton(TaskPrompts):
    ...
    dataset_info = """
    ```markdown
    This is a three-channel image dataset, consisting of images from different experiments and different settings - a heterogenous dataset of many different object types.  There is a particular focus on biological microscopy images, including cells, sometimes with nuclei labeled in a separate channel.
    ...
    The cell images have dimensions (B, L, W, C) = (batch, length, width, channel). The images provided must be in the format of standard ImageData object and must maintain channel dimensions and ordering.   
    """

    def get_task_details(self):
        return f"""
    All of you should work together to write {self.k_word} preprocessing functions that {self.if_advantage("maximize the reported advantages and ")}improve performance using OpenCV functions (APIs provided).
    ...
    def get_pipeline_metrics_info(self):
        return f"""
        A metric will be used to evaluate the performance of the pipeline.
    """
