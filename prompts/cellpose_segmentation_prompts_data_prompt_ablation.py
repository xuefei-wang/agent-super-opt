...
class CellposeSegmentationPromptsWithSkeleton(TaskPrompts):
    ...
    dataset_info = """
    ```markdown
    This is a three-channel image dataset.
    ...
    We want to increase the neural network tool's performance at segmenting objects that have high Intersection over Union (IoU) with the ground truth masks.
    The images have dimensions (B, L, W, C) = (batch, length, width, channel). To correctly predict masks, the images provided must be in the format of standard ImageData object and must maintain channel dimensions and ordering.   
    """
